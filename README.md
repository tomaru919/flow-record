...
- [x] DB Error: The operation has timed out
...
- [ ] vscodeのタイトルが"Code"になる
- [x] 日曜日を基準に1週間単位で稼働時間を表示できるようにする
- [x] アクティブウインドウの割合を表すグラフで少ない割合のウインドウをその他に分類する
- [x] アクティブウインドウの割合を表すグラフではChromeが5時間と表示されているが１日のパソコンの稼働時間を表すグラフには３時間半と表示される
- [ ] スリープしている状態が長いと2つ以上データが重複して保存されたり、スリープ解除した時間とは違うデータwake_timeに記録されたりする
- [ ] アプリが起動したあと、スリープとスリープ解除をするとおなじような時間がsleep_timeとwake_timeに記録される
- [x] アプリウインドウを開いたときリフレッシュする
- [x] データベースをsqliteに換装する
- [ ] 起動、シャットダウン、スリーブ、スリーブ解除をした時間をフロントエンドに表示する
- [ ] スリープ中、Bluetoothの切断や再接続があるとスリープとスリープ解除と誤認して記録される気がする
- [x] スリープ、スリープ解除、ブート、シャットダウン時はその詳細をログに残す
- [x] FlowRecordを日をまたいで実行していると二日目のデイリーアクティビティチャートが記録されない
- [x] アクティビティチャートのスリープ状態だった時間を見れるようにする
- [ ] githubからリリースできるようにする
- [ ] アクティブウィンドウの内訳も遡れるようにする
- [ ] スリープ時間がない場合、起動時間のバーの先頭を丸める
