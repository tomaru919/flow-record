...
- **Diagnostics (temporary)**: While investigating, added `MonitorService.LogPower(string)`, which appended timestamped lines to `%LocalAppData%\FlowRecord\power.log` — unlike `Debug.WriteLine` (invisible unless a debugger is attached), this file was inspectable after a real sleep/resume test on any machine. It confirmed the root cause (`wParam=0x8013`/`PBT_POWERSETTINGCHANGE` fired; `PBT_APMSUSPEND`/`RESUMEAUTOMATIC` never did, even on the S3-capable desktop) and that the fix worked (DB rows recorded correctly). Removed once confirmed, per user request, to keep the shipped code free of debug-only instrumentation.
